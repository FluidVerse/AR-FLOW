...
namespace InteractionMenus {
    ...
    public class InteractionMenuManager : MonoBehaviour {
        ...
        ///
        /// Can be set using <see cref="EnableInteractions"/> and <see cref="DisableInteractions"/>.
        /// </summary>
        public bool IsCheckingForInteractions { get; private set; } = true;
        ...
        /// <summary>
        /// Enables interactions and allows the interaction menu to be shown again.
        /// </summary>
        ...
        /// <summary>
        /// Disables interactions and hides the interaction menu for the given reason.
        ///
        /// Interactions can be enabled again by setting <see cref="IsCheckingForInteractions"/> to <c>true</c>.
        /// </summary>
        /// <param name="reason">Reason for disabling</param>
        public void DisableInteractions(DisableReason reason) {
            if (disableReason == DisableReason.None) {
                disableReason = reason; // don't override if reason already exists
            }
        ...
        public void OnMainMenuToggle(bool isShown) {
            if (isShown) {
                DisableInteractions(DisableReason.MainMenu);
            } else {
                if (disableReason == DisableReason.MainMenu) {
                    EnableInteractions(); // enable it again after entering main menu and clicking continue
                }
            }
        }
        ...
        public void OnCameraModeChanged(CameraType cameraType) {
            if (cameraType == CameraType.AerialView) {
                DisableInteractions(DisableReason.CameraMode);
            } else {
                if (disableReason == DisableReason.CameraMode) {
                    EnableInteractions();
                }
            }
        }
        ...
        /// <summary>
        /// Reasons for disabling the interaction menu and interactions in general. 
        /// </summary>
