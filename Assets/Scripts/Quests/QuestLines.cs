...
namespace Quests {
    ...
    public static class QuestLines {
        ...
        public static QuestLine TutorialLevel =>
            new QuestLine("Tutorial Level", "Tutorial description!", new List<QuestStage> {
                new QuestStage("Stage 1",
                    "<unused>",
                    new List<Quest> {
                        new Quest("Get an overview",
                            "Take a closer look at the pump and the electric motor.",
                            new List<IQuestTarget> {
                                new QuestTarget<object>(Pumpe1, UseDetailView),
                                new QuestTarget<object>(Elektromotor1, UseDetailView),
                            },
                            new List<IQuestInteraction> {
                                new QuestInteraction<object>(SecretObject, RevealObject)
                            }),
                        new Quest("Inspect valve",
                            "Take a closer look at the valve.",
                            new List<IQuestTarget> {
                                new QuestTarget<object>(Kugelhahn1, UseDetailView)
                            },
                            isOptional: true),
                        new Quest("Set valve", "Set the valve to the correct position.",
                            new List<IQuestTarget> {
                                new QuestTarget<int>(Kugelhahn1, SetValvePosition, value => value > 50)
                            },
                            new List<IQuestInteraction> {
                ...
                new QuestStage("Stage 2",
                    "<unused>",
                    new List<Quest> {
                        new Quest("LevelObjects Button 2",
                            "Click on LevelObjects Button 2",
                            new List<IQuestTarget> {
                                new QuestTarget<object>(LevelObjectsButton1, ButtonClicked)
                            })
                    })
            });
